...
def train(model_path, data_dir, output_dir):
  ...
  model.train_with_sgd(
      train_and_val_dataset, num_iter, learning_rate, batch_size=batch_size, print_every=100)
  t2 = time.perf_counter()
  print('Training takes %.2f seconds' % (t2 - t1))
  print(train_and_val_dataset['labels_train'][0])
...
#   print(test_dataset['labels_test'][0])
  saved_model_acc = np.mean(
      np.argmax(test_embeddings, axis=1) == test_dataset['labels_test'])
#   balanced_saved_model_acc = balanced_accuracy_score(test_dataset['labels_test'], np.argmax(test_embeddings, axis=1))
  print('Saved tflite model test accuracy: %.2f%%' % (saved_model_acc * 100))
#   print('balanced model test accuracy: %.2f%%' % (balanced_saved_model_acc * 100))
